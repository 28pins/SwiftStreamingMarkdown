...
struct ListItemContentWrapper<Content: View>: View {
  ...
  private func extractFirstFont() -> PlatformFont {
    ...
    if let font = firstPlatformFont(in: paragraphContents) {
      return font
    }
  ...
  private func firstPlatformFont(in attributedString: NSAttributedString) -> PlatformFont? {
    guard attributedString.length > 0 else { return nil }

    // Fast path: attribute at location 0
    if let font = attributedString.attribute(.font, at: 0, effectiveRange: nil) as? PlatformFont {
      return font
    }
